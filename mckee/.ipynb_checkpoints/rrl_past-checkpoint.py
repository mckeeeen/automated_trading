...
class RRL(nn.Module):
    ...
    * Ft: mono layer neural network:: tanh(wx + b + uFt-1)
    ...
    def fit(self, price_data, other_data=None):
    ...
    def _input_data(self, price_data, other_data=None):
        ...
        _price_diff_piece = []
        for i in range(int(self.m + 1), _price_diff.size()[0]):
    ...
    def _calculate(self):
        ...
        self.dUt_dRt = (self.Bt1 - self.At1 * self.Rt) / ((self.Bt1 - self.At1 ** 2) ** 1.5)
    ...
    def _record(self):
        ...
        self.sharperatio += self.eta * self.At / ((((1 - self.eta / 2) / (1 - self.eta)) ** 0.5) * ((self.Bt - self.At ** 2) ** 0.5))
        self.utility_seq.append(self.sharperatio.clone())
